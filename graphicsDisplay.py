...
OPPONENT_COLOR = formatColor(255.0/255.0,0.0/255.0,0.0/255.0)
...
class InfoPane:
    ...
    def drawPane(self):
        self.scoreText1 = text( self.toScreen(0, 0  ), self.textColor, "PACMAN SCORE:    0", "Times", self.fontSize, "bold")
        self.scoreText2 = text( self.toScreen(0, 12  ), self.textColor, "OPPONENT SCORE:    0", "Times", self.fontSize, "bold")
        self.scoreText3 = text( self.toScreen(0, 24 ), self.textColor, "GHOST SCORE:    0", "Times", self.fontSize, "bold")
    ...
    def updateScore(self, scores):
        changeText(self.scoreText1, "PACMAN SCORE: % 4d" % scores[0])       
        changeText(self.scoreText2, "OPPONENT SCORE: % 4d" % scores[1])
        changeText(self.scoreText3, "GHOST SCORE: % 4d" % scores[2])
...
class PacmanGraphics:
    ...
    def drawAgentObjects(self, state):
        ...
        for index, agent in enumerate(state.agentStates):
            ...
            elif agent.isOpponent:
                image = self.drawOpponent(agent, index)
                self.agentImages.append( (agent, image) )
            else:
    ...
    def swapImages(self, agentIndex, newState):
        ...
        elif newState.isOpponent:
            image = self.drawOpponent(newState, agentIndex)
            self.agentImages[agentIndex] = (newState, image )
        else:
    ...
    def update(self, newState):
        ...
        elif self.agentImages[agentIndex][0].isOpponent != agentState.isOpponent: 
            self.swapImages(agentIndex, agentState) 
        prevState, prevImage = self.agentImages[agentIndex]
        if agentState.isPacman or agentState.isOpponent:
        ...
        self.infoPane.updateScore(newState.scores)
    ...
    def drawOpponent(self, opponent, index):
        position = self.getPosition(opponent)
        screen_point = self.to_screen(position)
        endpoints = self.getEndpoints(self.getDirection(opponent))

        width = PACMAN_OUTLINE_WIDTH
        outlineColor = OPPONENT_COLOR
...
class FirstPersonPacmanGraphics(PacmanGraphics):
    ...
    def getPosition(self, ghostState):
        if not self.showGhosts and not ghostState.isPacman and not ghostState.isOpponent and ghostState.getPosition()[1] > 1:
            return (-1000, -1000)
        else:
